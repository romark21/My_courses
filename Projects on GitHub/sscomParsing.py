...
for i, url in enumerate(get_url(), 1):
    ...
    try:  # Проверяем соответствуют ли данные нашему запросу, если да присваиваем
        year = data.find('td', {'id': 'tdo_18'}).text  # , если да присваиваем, тогда добавляем в переменную
    except AttributeError:  # Если возникнет ошибка, тогда присвоим переменной другое значение и цикл не прирвётся
        year = "--------"
